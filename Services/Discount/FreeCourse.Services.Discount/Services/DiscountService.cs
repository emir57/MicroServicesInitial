using Dapper;
using FreeCourse.Services.Discount.Constants;
...
public sealed class DiscountService : IDiscountService
{
    ...
    public async Task<Response<NoContent>> DeleteAsync(int id)
    {
        ...
        return Response<NoContent>.Fail(DiscountServiceMessages.DeletedFail, 500);
    }
    ...
    public async Task<Response<Models.Discount>> GetByCodeAndUserId(string code, string userId)
    {
        Models.Discount discount = await _dbConnection.QueryFirstOrDefaultAsync<Models.Discount>("select * from discount where code=@Code and userid=@UserId"
            , new { code, userId });
        if (discount == null)
            return Response<Models.Discount>.Fail(DiscountServiceMessages.DiscountIsNotFound, 404);
    ...
    public async Task<Response<NoContent>> SaveAsync(Models.Discount discount)
    {
        ...
        if (saveStatus > 0)
            return Response<NoContent>.Success(204);
        return Response<NoContent>.Fail(DiscountServiceMessages.AddedFail, 500);
    }
    ...
    public async Task<Response<NoContent>> UpdateAsync(Models.Discount discount)
    {
        ...
        if (updateStatus > 0)
            return Response<NoContent>.Success(204);
        return Response<NoContent>.Fail(DiscountServiceMessages.UpdatedFail, 500);
    }
}
